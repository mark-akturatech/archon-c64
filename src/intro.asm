...
entry:
    ...
    lda _data__logo_sprite_color_list
    sta SPMC0
    sta SPMC1
    // Configure the starting intro state function.
    lda #<_state__scroll_title
...
!icon_loop:
    ldy _data__icon_id_list,x
    sty common.icon.type
    lda board.icon.init_matrix,y
    sta common.icon.offset,x
    jsr common.sprite_initialize
    lda _flag__is_icon_sprite_mirrored_list,x // Invert frames for icons pointing left
...
!loop: // Copy remaining 206 bytes
    ...
    sta _ptr__sprite_mem
    ldx #NUM_SPRITES
!loop:
    txa
    asl
    tay
    lda _ptr__sprite_mem
    sta SPTMEM,x
    dec _ptr__sprite_mem
    lda _data__logo_sprite_color_list,x
    sta SP0COL,x
    lda _pos__logo_sprite_x_list,x
    sta SP0X,y
    sta common.sprite.curr_x_pos,x
    lda _pos__logo_sprite_y_list,x
    sta SP0Y,y
...
// AA8B
_state__draw_freefall_logo:
    lda #FLAG_ENABLE
    sta _flag__string_pos_ctl
...
!loop:
    ...
    sty _idx__string
!loop:
    stx _data__curr_line
    jsr _screen_draw_text
    ldx _data__curr_line
    dex
    dec _idx__string
    ldy _idx__string
    cpy #$04
    ...
    jsr _screen_draw_text
    //
    dec _idx__string // Set to pointer next string to display in next state
    // Start scrolling Avatar logo colors
    lda #<_state__avatar_color_scroll
    sta ptr__substate_fn
    lda #>_state__avatar_color_scroll
    sta ptr__substate_fn+1
    jmp common.complete_interrupt
...
// Notes:
// - The message offset is used to read the string memory location from `_ptr__intro_string_list` (message offset is
...
_screen_draw_text:
    ldy _idx__string
    lda _data__string_color_list,y
    sta _data__curr_color
    tya
    asl
    tay
    lda _ptr__intro_string_list,y
    sta FREEZP
    lda _ptr__intro_string_list+1,y
    sta FREEZP+1
    ldy #$00
    jmp _screen_calc_start_addr
...
_screen_calc_start_addr:
    lda _flag__string_pos_ctl
...
!next:
    ...
    sta FREEZP+2
    sta FORPNT
    jmp _screen_write_chars

// AB4F
_state__show_authors:
...
!loop:
    ...
    lda #$07
    sta common.sprite.y_move_counter
    lda _flag__sprite_y_direction
    eor #$FF
    sta _flag__sprite_y_direction
!next:
    dec common.sprite.x_move_counter
    bne _state__avatar_color_scroll
    lda #$1C
    sta common.sprite.x_move_counter
    lda _flag__sprite_x_direction
    eor #$FF
    sta _flag__sprite_x_direction
...
_state__chase_scene:
    lda _flag__are_sprites_initialized
...
!skip:
    ...
    lda #FLAG_ENABLE
    sta _flag__are_sprites_initialized // Set sprites intiialised flag
...
!loop:
    ...
    lsr
    sta common.sprite.curr_x_pos,x
    lda _data__icon_sprite_color_list,x
    sta SP0COL,x
    dex
    bpl !loop-
    jmp _animate_icons
...
!loop:
    ...
    tay
    lda common.sprite.curr_x_pos,x
    cmp _pos__icon_sprite_final_x_list,x
    beq !next+
    clc
    adc _flag__icon_sprite_direction_list,x
    sta common.sprite.curr_x_pos,x
...
!skip:
    ...
    clc
    adc _ptr__icon_sprite_mem_list,x
    sta SPTMEM,x
!next:
    dex
    bpl !loop-
...
ptr__substate_fn_list:
    .word _state__draw_freefall_logo, _state__show_authors, _state__avatar_bounce, _state__chase_scene
    .word _state__end_intro

// A97A
// Initial sprite y-position for intro logo sprites.
_pos__logo_sprite_y_list: .byte $ff, $ff, $ff, $ff, $30, $30, $30

// A981
// Initial sprite x-position for intro logo sprites.
_pos__logo_sprite_x_list: .byte $84, $9c, $b4, $cc, $6c, $9c, $cc

// A988
// Initial color of intro logo sprites.
_data__logo_sprite_color_list: .byte YELLOW, YELLOW, YELLOW, YELLOW, WHITE, WHITE, WHITE

// ADAA:
// Icon IDs of pieces used in chase scene.
_data__icon_id_list: .byte GOBLIN, GOLEM, TROLL, KNIGHT

// ADAE:
// Direction flags of intro sprites ($80=invert direction).
_flag__is_icon_sprite_mirrored_list: .byte FLAG_DISABLE, FLAG_DISABLE, FLAG_ENABLE, FLAG_ENABLE

// ADB2
// Direction of each icon sprite (FF=left, 01=right).
_flag__icon_sprite_direction_list: .byte $FF, $FF, $01, $01

// ADB6
// End position of each intro icon sprite.
_pos__icon_sprite_final_x_list: .byte $00, $00, $AC, $AC

// ADBA
// Screen pointer sprite offsets for each icon.
_ptr__icon_sprite_mem_list:
...
_data__icon_sprite_color_list: .byte YELLOW, LIGHT_BLUE, YELLOW, LIGHT_BLUE

// A8A5
// Pointer to string data for each string.
_ptr__intro_string_list: 
...
_flag__are_sprites_initialized: .byte $00

// BF23
// Current sprite Y position of bouncing Archon sprite.
_pos__sprite_y: .byte $00
